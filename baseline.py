...
tf.app.flags.DEFINE_integer('hidden_dim', 300, 'dimension')
tf.app.flags.DEFINE_integer('word_dim', 300, 'dimension')
...
class Baseline:
    ...
    def pos_index(self, x, thre=15):
        if x < -thre:
            return 0
        if x >= -thre and x <= thre:
            return x + thre + 1
        if x > thre:
            return 2 * thre + 2
    ...
    def run_test(self, sess, test_batchers):
        ...
        all_labels = []
        if self.bag:
            answer_file = 'answer/bag_relation_answer.txt'
        else:
            answer_file = 'answer/sent_relation_answer.txt'

        with open(os.path.join(self.data_path, answer_file), 'r') as fr:
            for line in fr:
                rel = [0] * self.num_classes
                bag_id, types = line.strip().split('\t')
